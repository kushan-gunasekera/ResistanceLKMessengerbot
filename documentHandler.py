...
def privateDocument(bot, message):
    if (common.chechStatus(message.from_user.id) == False):
        try:
            ...
            bot.send_message(chat_id=admin, text='Private Document Forward Failed from agent ' + str(common.getName(message.from_user)), parse_mode='HTML')
        ...
        for chatAdmin in bot.get_chat_administrators(chat_id=groupID):
            if(message.from_user.id==chatAdmin.user.id and chatAdmin.user.is_bot==False):
                try:
                    ...
                    try:
                        bot.send_message(chat_id=admin, text='Reply Document Send Failed to agent ' + str(common.getName(message.reply_to_message.forward_from)), parse_mode='HTML')
                    except:
                        bot.send_message(chat_id=admin, text='Trying to reply as a Document to the BOT message by ' + str(common.getName(message.from_user)))