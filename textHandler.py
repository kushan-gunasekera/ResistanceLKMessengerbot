    ...
    if(common.chechStatus(message.from_user.id)==False):
        try:
            ...
            bot.send_message(chat_id=message.chat.id,
                             text='<b>Your Submission has Successfully Submitted</b> ' + str(common.getName(message.from_user)),
                             parse_mode='HTML')
        except:
            bot.send_message(chat_id=admin, text='Private Text Forward Failed from agent ' + str(common.getName(message.from_user)), parse_mode='HTML')
        ...
        for chatAdmin in bot.get_chat_administrators(chat_id=groupID):
            if(message.from_user.id==chatAdmin.user.id and chatAdmin.user.is_bot==False):
                try:
                    ...
                    try:
                        bot.send_message(chat_id=admin, text='Reply Text Send Failed to agent ' + str(common.getName(message.reply_to_message.forward_from)), parse_mode='HTML')
                    except:
                        bot.send_message(chat_id=admin, text='Trying to reply as a Text to the BOT message by ' + str(common.getName(message.from_user)))